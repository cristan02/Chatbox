...
function Signup() {
    ...
    const istext = (e) => {
        if((e.target.value.at(-1) >= 'a' && e.target.value.at(-1) <= 'z') || (e.target.value.at(-1) >= 'A' && e.target.value.at(-1) <= 'Z') || (e.target.value.at(-1) >= ' '))
    ...
    const register = (e) => {
        ...
        .then((res) => {
            alert(res.data)
            if(res.data == 'Account Successfully created'){
  ...
  return (
    <div className='bg-[#EDFBFF] w-full h-screen flex justify-center items-center'>
        <div className='py-16 px-16 bg-white drop-shadow-md rounded-md'>
            <form className='flex  flex-col items-center w-96' ref={formRef} onSubmit={register}>
                ...
                <div className='w-full relative p-0 grid '>
                    <input type='text' placeholder='Enter Name' className='px-3 py-2 w-full border rounded pr-9' value={name} onChange={(e)=>{istext(e)}}></input>
                </div>
                <p className='p-2'></p>

                <div className='w-full relative p-0 grid '>
                    <input type="email" placeholder='Enter Email' className='px-3 py-2 w-full border rounded pr-9' value={email} onChange={(e)=>{setEmail(e.target.value)}}></input>
                </div>
                <p className='p-2'></p>

                <div className='w-full relative p-0 grid '>
                    <input type={hidePass? "password" : "text"} placeholder='Create Password' className='px-3 py-2 w-full border rounded pr-9' value={password} onChange={(e)=>{setPassword(e.target.value)}}></input>
                ...
                <p className='p-2'></p>
                <button className='bg-[#FDC666] hover:bg-[#fdc032] font-semibold px-2 py-2 rounded-md w-full'>SignUp</button> 

                <p className='p-2'></p>
                <p className='text-center font-semibold'>~ Or SignUp with ~</p>
                <p className='p-2'></p>

                <div className='grid grid-cols-2 gap-6 w-full'>
                    <div className='border rounded-md px-3 py-2 flex items-center justify-center hover:bg-sky-200 cursor-pointer'>
                ...
                <p className='p-2'></p>
                <p className=''>Already have a Team? 
                    <Link to='/signin'>
                        <button className='font-bold pl-1'> SignIn</button>
                    </Link>
                </p>
